...
- (void)willActivate {
    ...
    [formatTime setDateFormat:@"M/d"];
    NSString *weekday = [formatWeekday stringFromDate:currentDate];
    NSString *time = [formatTime stringFromDate:currentDate];
    
    [self loadData];
    
    [_topLabel setText:@"No lunch today"];

    if (!([weekday isEqualToString:@"Saturday"] || [weekday isEqualToString:@"Sunday"])) {
        [_topLabel setText:[NSString stringWithFormat:@"%@ %@\nToday's lunch is:", weekday, time]];
    } else {
        [_topLabel setText:@"No lunch today"];
        [_mainLabel setText:@""];
    }
